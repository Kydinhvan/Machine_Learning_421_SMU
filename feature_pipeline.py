...
Pipeline:
    ...
    2. exclude_test_users()    — optional, remove overlapping users (trial phase)
    3. compute_item_stats()    — freeze item-level stats from training only
    4. build_features()        — engineer per-user features
...
def _user_count_bounds(median_count: int) -> tuple[int, int]:
    """Return safe lower/upper bounds for synthetic per-user item counts."""
    lower = max(1, median_count // 2)
    upper = max(lower + 1, median_count * 2)
    return lower, upper
...
def generate_synthetic_anomalies(
    ...
    low_n, high_n = _user_count_bounds(median_count)

    # Type 1: Random rater (uniform random ratings)
    for _ in range(n_per_type):
        n = rng.randint(low_n, high_n)
...
# ── 2. Exclude test users (trial phase only) ──────────────────────────────
...
def build_features(
    ...
    pop_denom = XX_pop.groupby("user")["item_popularity"].sum().replace(0, np.nan)
    pwf = XX_pop.groupby("user")["pop_weighted_rating"].sum() / pop_denom
...
# Features computed purely from a user's own ratings.
# These do not depend on frozen training item stats and are more shift-resistant.
ROBUST_FEATURES = [
